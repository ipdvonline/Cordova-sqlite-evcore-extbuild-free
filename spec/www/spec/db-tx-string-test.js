  ...
  for (var i=0; i<scenarioCount; ++i) {
    ...
    describe(scenarioList[i] + ': tx string test(s)', function() {
      ...
      describe(suiteName + 'U+0000 character tests', function() {
            ...
            tx.executeSql('SELECT HEX(?) AS hexvalue', ['abcd'], function (tx_ignored, rs1) {
              ...
              tx.executeSql('SELECT HEX(?) AS hexvalue', ['a\u0000cd'], function (tx_ignored, rs2) {
                ...
                if (isWindows)
                  ...
                  expect(resultRow2.hexvalue).toBe('61303030306364'); // (XXX UTF-8 with XXX ENCODING BUG REPRODUCED on Android [evcore])
                else
                ...
                if (isWindows)
                  ...
                  expect(resultRow2.hexvalue.length).toBe(14);  // (XXX UTF-8 with ENCODING BUG REPRODUCED on Android [evcore])
                else
                ...
                tx.executeSql('SELECT HEX(?) AS hexvalue', ['e\0gh'], function (tx_ignored, rs3) {
                  ...
                  if (isWindows)
                    ...
                    expect(resultRow3.hexvalue).toBe('65303030306768'); // (XXX UTF-8 with ENCODING BUG REPRODUCED on Android [evcore])
                  else
        ...
        it(suiteName + 'INLINE HEX encoding test with U+0000 (\\0) [XXX HEX ENCODING BUG REPRODUCED on default Android NDK access implementation (using evcore-native-driver), SQL ERROR reported otherwise; default sqlite encoding: UTF-16le on TBD, UTF-8 otherwise]', function (done) {
            ...
            tx.executeSql('SELECT HEX("efgh") AS hexvalue', [], function (tx_ignored, rs1) {
              ...
              tx.executeSql("SELECT HEX('e\u0000gh') AS hexvalue", [], function (tx_ignored, rs2) {
                ...
                if (!isWebSql && !isWindows && isAndroid && !isImpl2)
                  expect(resultRow2.hexvalue).toBe('65303030306768'); // XXX UTF-8 ENCODING BUG REPRODUCED on default Android evcore implementation (NDK)
                else
                ...
                // default encoding
                if (!isWebSql && !isWindows && isAndroid && !isImpl2)
                  expect(resultRow2.hexvalue.length).toBe(14);  // XXX INCORRECT LENGTH on default Android evcore implementation (NDK)
                else
                  expect(resultRow2.hexvalue.length).toBe(expected_hexvalue_length);
        ...
        it(suiteName + 'U+0000 string parameter manipulation test [XXX TBD ENCODING ISSUE REPRODUCED on default Android evcore implementation (NDK), TRUNCATION ISSUE REPRODUCED on (WebKit) Web SQL & plugin on multiple platforms]', function(done) {
          ...
          db.transaction(function(tx) {
            tx.executeSql('SELECT UPPER(?) AS uppertext', ['a\u0000cd'], function(tx_ignored, rs) {
              ...
              if ((isWebSql && isAndroid && (/Android 4/.test(navigator.userAgent))) ||
                ...
                expect(rs.rows.item(0).uppertext).toBe('A0000CD'); // XXX ENCODING ISSUE REPRODUCED on default Android evcore implementation (NDK)
              else
      ...
      describe(suiteName + 'control character tests', function() {
        ...
        it(suiteName + 'string vertical tab test (inline vs argument parameter value) [XXX ENCODING ISSUE REPRODUCED on default Android evcore implementation (NDK); default sqlite HEX encoding: UTF-6le on Windows & Android 4.1-4.3 (WebKit) Web SQL, UTF-8 otherwise]', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql("SELECT UPPER('first\vsecond') AS uppertext", [], function(tx_ignored, rs1) {
              ...
              expect(rs1.rows).toBeDefined();
              expect(rs1.rows.length).toBe(1);
              // XXX ENCODING ISSUE REPRODUCED on default Android evcore implementation (NDK)
              if (!isWebSql && !isWindows && isAndroid && !isImpl2) // XXX
                expect(rs1.rows.item(0).uppertext).toBe('FIRST000BSECOND'); // XXX ENCODING ISSUE REPRODUCED on default Android evcore implementation (NDK)
              else
              ...
              tx.executeSql("SELECT HEX(?) AS myResult", ['A\v1'], function(tx_ignored, rs2) {
                ...
                if (isWindows || (isWebSql && isAndroid && /Android 4.[1-3]/.test(navigator.userAgent)))
                  ...
                  expect(rs2.rows.item(0).myResult).toBe('413030306231'); // XXX ENCODING ISSUE REPRODUCED on default Android evcore implementation (NDK)
                else
        ...
        it(suiteName + 'string form feed test (inline vs argument parameter value) [XXX ENCODING ISSUE REPRODUCED on default Android evcore implementation (NDK); default sqlite HEX encoding: UTF-6le on Windows & Android 4.1-4.3 (WebKit) Web SQL, UTF-8 otherwise]', function(done) {
          if (isWP8) pending('BROKEN on WP(8)'); // [BUG #202] UNICODE characters not working with WP(8)
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql("SELECT UPPER('first\fsecond') AS uppertext", [], function(tx_ignored, rs1) {
              ...
              // XXX ENCODING ISSUE REPRODUCED on default Android evcore implementation (NDK)
              if (!isWebSql && !isWindows && isAndroid && !isImpl2) // XXX
                expect(rs1.rows.item(0).uppertext).toBe('FIRSTSECOND'); // XXX ENCODING ISSUE REPRODUCED on default Android evcore implementation (NDK)
              else
              ...
              tx.executeSql("SELECT HEX(?) AS myResult", ['A\f1'], function(tx_ignored, rs2) {
                ...
                if (isWindows || (isWebSql && isAndroid && /Android 4.[1-3]/.test(navigator.userAgent)))
                  ...
                  expect(rs2.rows.item(0).myResult).toBe('4131');
                else
        ...
        it(suiteName + 'string backspace test (inline vs argument parameter value) [XXX ENCODING ISSUE REPRODUCED on default Android evcore implementation (NDK); default sqlite HEX encoding: UTF-6le on Windows & Android 4.1-4.3 (WebKit) Web SQL, UTF-8 otherwise]', function(done) {
          if (isWP8) pending('BROKEN on WP(8)'); // [BUG #202] UNICODE characters not working with WP(8)
          // XXX TBD ???:
      ...
      describe(suiteName + 'UTF-8 multi-byte character string binding & manipulation tests', function() {
        ...
        it(suiteName + 'string manipulation test with UTF-8 3-byte Samaritan character Bit (U+0801)', function(done) {
          // XXX KNOWN CRASH on Android (default evcore-native-driver NDK database access implementation)
          // ref: litehelpers/Cordova-sqlite-evcore-extbuild-free#37
          if (!isWebSql && !isWindows && isAndroid && !isImpl2) pending('XXX KNOWN CRASH on Android (default evcore-native-driver NDK database access implementation)');
        ...
        // XXX KNOWN ENCODING AND CRASH ISSUE on default
        // Android evcore NDK access implementation:
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('SELECT HEX(?) AS myresult', ['@𐌱'], function(ignored, rs1) {
              ...
              if (isWindows || (isWebSql && isAndroid && /Android 4.[1-3]/.test(navigator.userAgent)))
                ...
                expect(rs1.rows.item(0).myresult).toBe('40F0908C2D62312D'); // XXX ENCODING BUG REPRODUCED on default Android evcore NDK implementation
              else
              ...
              tx.executeSql("SELECT HEX('@𐌱') AS myresult", null, function(ignored, rs2) {
                ...
                if (isWindows || (isWebSql && isAndroid && /Android 4.[1-3]/.test(navigator.userAgent)))
                  ...
                  expect(rs2.rows.item(0).myresult).toBe('40F0908C2D62312D'); // XXX ENCODING BUG REPRODUCED on default Android evcore NDK implementation
                else
        ...
        it(suiteName + 'emoji HEX test: SELECT HEX("@\\uD83D\\uDE03!") [\\u1F603 SMILING FACE (MOUTH OPEN)] [XXX ENCODING BUG REPRODUCED on default Android SQLite3 NDK build (using Android-sqlite-evcore-native-driver) on Android 4.x/5.x; default sqlite HEX encoding: UTF-6le on Windows & Android 4.1-4.3 (WebKit) Web SQL, UTF-8 otherwise]', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('SELECT HEX(?) AS hexvalue', ['@\uD83D\uDE03!'], function(tx_ignored, rs) {
              ...
              if (isWindows || (isWebSql && isAndroid && /Android 4.[1-3]/.test(navigator.userAgent)))
                ...
                expect(rs.rows.item(0).hexvalue).toBe('40F09F982D38332D21'); // XXX ENCODING BUG REPRODUCED on default Android evcore NDK implementation
              else
